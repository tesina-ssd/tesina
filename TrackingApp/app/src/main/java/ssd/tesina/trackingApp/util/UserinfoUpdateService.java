...
import ssd.tesina.trackingApp.activities.MainActivity;
...
public class UserinfoUpdateService extends Service {
        ...
        final BroadcastReceiver br = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (intent.getAction().equals(SMS_RECEIVED)) {
                    ...
                    keyword = sharedPreferences.getString(Constants.TEXT_KEYWORD, "").toLowerCase();

                    if ((keyword.length() == 0) || (!Constants.SMS_ENABLE)) {
                    ...
                    serviceIntent.putExtra(Constants.PHONE_NUMBER, list.get(0).getOriginatingAddress());
                    serviceIntent.putExtra(Constants.WHO_CALLING, Constants.LOCATION_MSG);
    ...
    private Runnable mToastRunnable = new Runnable() {
        @Override
        public void run() {
            Constants.IS_TRACKING_SERVICE_WORKING =true;
            Log.d("time-",""+time);
            if(time!=0){
                    date= new Date();
                    ...
                    if(systemTime<time){
                        ...
                        if(systemTime>timeConnectedMsg){
                            if(!msgConnectedSent){
                                ...
                                sendEmergencyMsgs(Constants.CONNECTED_PHONE_NUMBER);
                        ...
                        if(systemTime>timeAlarmMsg){
                            if(!msgAlarmSent){
                                msgAlarmSent=true;
                                sendEmergencyMsgs(Constants.ALARM_PHONE_NUMBER);
    ...
    private void sendEmergencyMsgs(String phone){
        ...
        serviceIntent.putExtra(Constants.PHONE_NUMBER,phone);
        serviceIntent.putExtra(Constants.WHO_CALLING, Constants.EMERGENCY_MSG);
        if(alreadyMsgSent){
            serviceIntent.putExtra(Constants.BOOL_CONNECTED_MSG,false);
            alreadyMsgSent=true;
        }else{
            serviceIntent.putExtra(Constants.BOOL_CONNECTED_MSG,true);
        }
        serviceIntent.putExtra(Constants.BOOL_ALARM_MSG,msgAlarmSent);
