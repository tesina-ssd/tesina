...
public class SmsSenderService extends JobIntentService {
    ...
    protected void onHandleWork(Intent intent) {
       // Log.d(TAG, "onHandleIntent");
        ...
        this.phoneNumber = Objects.requireNonNull(intent.getExtras()).getString(Constants.PHONE_NUMBER);
        this.caller = intent.getExtras().getString(Constants.WHO_CALLING);
        assert caller != null;
        if(caller.equals(Constants.EMERGENCY_MSG)){
            this.connectedSms=  intent.getExtras().getBoolean(Constants.BOOL_CONNECTED_MSG);
            this.alarmSms = intent.getExtras().getBoolean(Constants.BOOL_ALARM_MSG);
        }
    ...
    private void readSettings() {
        SharedPreferences settings = context.getSharedPreferences(Constants.SHARED_PREFS, MODE_PRIVATE);

        locationSms = settings.getBoolean(Constants.SWITCH_LOCATION_SMS, false);
        googleMapsSms = settings.getBoolean(Constants.SWITCH_GOOGLEMAPS_SMS, false);
        singleSms = settings.getBoolean(Constants.SWITCH_SINGLE_SMS, false);
